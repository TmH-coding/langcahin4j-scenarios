package com.langchain4j.scenarios.scenario5;
...
/**
 * 内容创作应用启动类 - Spring Boot 应用入口点
 *
 * 职责说明：
 * - 作为 Spring Boot 应用的主入口点
 * - 配置组件扫描范围包括 common 和 scenario5 包
 * - 启动内嵌 Tomcat 服务器
 * - 初始化 Spring 应用上下文
 * - 加载所有配置和依赖注入
 *
 ...
 * 3. 编译后运行：java -jar scenario-5-content-creator.jar
 * 4. Docker 容器中运行
 *
 * 配置说明：
 ...
 *   * 包括 com.langchain4j.scenarios.scenario5（业务模块）
 *   * 确保所有 @Component、@Service、@Controller 等被发现
 *
 * 启动流程：
 * 1. JVM 加载 ContentCreatorApplication 类
 * 2. 执行 main 方法
...
public class ContentCreatorApplication {
    ...
    public static void main(String[] args) {
        SpringApplication.run(ContentCreatorApplication.class, args);
    }
}
